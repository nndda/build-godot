...
if [[ "$EXPORT_PLATFORM" == "android" ]]; then
  ...
  scons platform=$EXPORT_PLATFORM arch=$EXPORT_ARCH lto=full production=yes target=template_release debug_symbols=no $xtra_flags generate_android_binaries=yes
